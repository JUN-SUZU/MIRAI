    ...
    if (method === 'GET') {
        ...
        req.on('end', () => {
            ...
            try {
                ...
                if (url === '/login/api/') {
                    ...
                    if (!db.serverData[data.serverID]) {
                        ...
                        db.serverData[data.serverID] = {
                            ...
                            excluded: [],
                            inviteProxy: false,
                            inviteURL: Math.random().toString(36).slice(-8),
                        };
                    }
                    else if (!guild.members.cache.get(data.userID).permissions.has(PermissionsBitField.Flags.Administrator)) {
                        res.writeHead(403, { 'Content-Type': 'application/json' });
                ...
                else if (url === '/setting/server/check_invite/api/') {
                    const serverID = db.lookUpLinkCode(data.linkCode);
                    if (!serverID || serverID === data.guildId) {
                        res.writeHead(200, { 'Content-Type': 'application/json' });
                        res.end(JSON.stringify({ result: 'success' }));
                        return;
                    }
                        ...
                        res.writeHead(200, { 'Content-Type': 'application/json' });
                        res.end(JSON.stringify({ result: 'fail', message: 'この招待リンクは他のサーバーで使用されています。' }));
                        return;
                    }
                }
                else if (url === '/invite/api/') {
                    const linkCode = data.linkCode;
                    const serverID = db.lookUpLinkCode(linkCode);
                    function returnFail() {
                        res.writeHead(403, { 'Content-Type': 'application/json' });
                        res.end(JSON.stringify({ result: 'fail', message: '無効な招待リンクまたは、招待リンクの作成に失敗しました。' }));
                        return;
                    }
                    if (!serverID) {
                        returnFail();
                    ...
                    client.guilds.cache.fetch(serverID).then(async guild => {
                        ...
                        const invite = await guild.channels.cache.filter(c => c.type === ChannelType.GuildText).first().createInvite({
                            maxAge: 30, // 30秒で期限切れ
                            maxUses: 1, // 1回のみ使用可能
                            unique: true,
                            reason: 'One-time invite link generated by MIRAI'
                        }).catch(e => {
                            console.error(e);
                            returnFail();
                        });
                        ...
                        db.linkcache[invite.code] = {
                            ip: ipadr,
                            date: new Date().toLocaleString(),
                            vpn: db.ipData[ipadr].vpn
                        };
                        res.writeHead(200, { 'Content-Type': 'application/json' });
                        res.end(JSON.stringify({ result: 'success', inviteLink: invite.url }));
                    }).catch(e => {
                        returnFail();
                    });
                }
...
client.on('guildMemberAdd', async (member) => {
    ...
    // 招待リンクの使用回数が一致しないものを探す
    const invites = await member.guild.invites.fetch();
    const usedInvite = invites.find(i => inviteList[i.code] && i.uses > inviteList[i.code].uses);
    if (usedInvite) {
        db.accountData[member.user.id].sessionLogs = db.accountData[member.user.id].sessionLogs || [];
        db.accountData[member.user.id].sessionLogs.push(db.linkcache[usedInvite.code] || {});
        db.write('account');
        inviteList[usedInvite.code].uses = usedInvite.uses;
    }
});
